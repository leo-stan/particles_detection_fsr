...
# Create point cloud out of image representation
if len(output[0,0,0]) > 10: # For case where multi echo is present (even if maybe not used)
    frames = np.zeros([len(output), output[0].shape[1]*output[0].shape[0], output.shape[3]-2])
else:
    frames = np.zeros([len(output), output[0].shape[1] * output[0].shape[0], output.shape[3] - 1])
for i, image in enumerate(output):
    for col in range(image.shape[1]):
        for row in range(image.shape[0]):
            if len(output[0,0,0]) > 10:
                frames[i, row+col*image.shape[0], :5] = image[row, col, 1:6]
